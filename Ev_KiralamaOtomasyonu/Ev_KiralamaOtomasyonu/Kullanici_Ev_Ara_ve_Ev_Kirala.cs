...
namespace Ev_KiralamaOtomasyonu
{
    public partial class Kullanici_Ev_Ara_ve_Ev_Kirala : Form
    {
        ...
        private void Kullanici_Ev_Ara_ve_Ev_Kirala_Load(object sender, EventArgs e)
        {
            ...
            picBoxEvFoto.Width = 208;
            picBoxEvFoto.Height = 208;
        ...
        private void button4_Click(object sender, EventArgs e)   // Kirala butonu
        {
            ...
            if (txtIlanNo.Text.Length == 11)
            {
                ...
                {
                    baglanti.Open();
                    SqlCommand kiralikara_sorgu = new SqlCommand("select * from Tbl_evler where ilanno='" + txtIlanNo.Text + "'", baglanti);  // Aranan ilan numarasına ait evin seçilmesi için 'select' sorgusu yazılır
                    SqlDataReader verioku = kiralikara_sorgu.ExecuteReader();  // Sorgudan elde edilen veriler DataReader'a aktarılır
                    while (verioku.Read())
                    {
                        if (verioku.GetValue(11).ToString() == "True") // Eğer seçilen evin SQL tablosundaki 'kiralik' değişkeni 'true' ise, yani ev kiralıksa
                        {
                            kiralik_durumu = true;   // Evin kiralık olduğunu belirlemek için kullandığımız kiralik_durumu değişkeni 'true' olarak değiştirilir.
                        }
                        break;
                    }
                    baglanti.Close();
                    if (kiralik_durumu == false)   // Eğer aranan ev kiralık değilse
                    {
                        lblGenelKosullar.Visible = true;    // Form ilk açıldığında kiralama yapılacak işlemin gözükmediği kısımlar Visible özelliği yardımıyla görünür hale getirildi
                        ...
                        {
                            baglanti.Open();
                            SqlCommand sozlesme_sorgu = new SqlCommand("select kosullar from Tbl_sozlesme", baglanti);  // Tbl_sozlesme tablosundan sözleşme maddelerini seçme sorgusu
                            SqlDataReader sozlesmeoku = sozlesme_sorgu.ExecuteReader();  // Sorgunun çalışması sağlandı
                            while (sozlesmeoku.Read())
                            {
                                richTxtSozlesme.Text += sozlesmeoku["kosullar"].ToString() + "\n";  // Sözleşme maddeleri richTxtSozlesme richtextbox'ına yazıldı
                            }
                            baglanti.Close();
                        }
                        catch (Exception errormsg)
        ...
        private void button6_Click(object sender, EventArgs e)    // Sözleşmeyi onaylama butonu
        {
            ...
            DateTime updated_date = date.AddMonths((int.Parse(comboBoxAy.Text)));    // comboBoxAy'da seçilen aya göre sözleşme tarihi belirlenir
            try
            {
                baglanti.Open();
                SqlCommand kiralikguncelle = new SqlCommand("update Tbl_evler set kiralik='" + kiralik_mi + "' where ilanno='" + txtIlanNo.Text + "'", baglanti);   // İlgili evin Tbl_evler tablosundaki kaydının 'kiralik' değişkeni 1 olarak değiştirilir, yani 'True'ya çevrilir. Yani evin kiralandığı belirlenir
                kiralikguncelle.ExecuteNonQuery();  // Yukarıda yazılan sorgunun gerçekleştirilmesi sağlandı
                baglanti.Close();
                try
                {
                    baglanti.Open();
                    SqlCommand kiraliktablo = new SqlCommand("insert into Tbl_kiralananEvler values ('" + Uye_Giris_Ekrani.tcno +  "', '" + txtIlanNo.Text  + "', '" + txtIl.Text + "', '" + txtIlce.Text + "', '" + Uye_Giris_Ekrani.ad + "', '" + Uye_Giris_Ekrani.soyad + "', '" + updated_date.ToString("yyyy/MM/dd") + "' )", baglanti);   // Evi kiralayan kişi artık bir kiracı olduğundan dolayı, bu kişinin bilgileri ve kiraladığı evin bilgileri Tbl_kiralananEvler tablosuna 'insert' komutu yardımıyla kaydedilir
                    kiraliktablo.ExecuteNonQuery();  // Yukarıda yazılan sorgunun gerçekleşmesi sağlandı
                    baglanti.Close();
                }
                catch (Exception errormsg)
                {
