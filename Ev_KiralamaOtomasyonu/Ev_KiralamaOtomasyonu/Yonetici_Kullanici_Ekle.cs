    ...
    public partial class Yonetici_Kullanici_Ekle : Form
    {
        ...
        private void kullanici_listele()
        {
            try
            {
                ...
                SqlDataAdapter listele = new SqlDataAdapter("select tcno AS[TC KİMLİK NO], ad AS[ADI], soyad AS[SOYADI], cinsiyet AS[CİNSİYETİ], kullaniciadi AS[KULLANICI ADI], parola AS[PAROLA], yetki AS[YETKİ], meslek AS[MESLEK], dogumtarihi AS[DOĞUM TARİHİ] from Tbl_kullanicilar order by ad ASC", baglanti);  // Tbl_kullanicilar tablosundan bütün değişkenlere ait veriler çekildi
                DataSet hafiza = new DataSet();  // Çekilen bu verilerin tutulabilmesi için DataSet sınıfından bir örnek oluşturuldu
                listele.Fill(hafiza);  // listele örneğinden gelen veriler DataSet'in içine dolduruldu
                dataGridView1.DataSource = hafiza.Tables[0];  // DataSet'teki veriler DataGridView'e aktarıldı.
                baglanti.Close();  // Bağlantı kapatıldı.
            }
            catch (Exception errormsg)  // Yapılan işlem sırasında oluşacak herhangi bir problemin önüne geçmek için try-catch bloğu kullanıldı
            {
        ...
        public void Kullanici_Ekle_Temizle()   // textboxları ve picturebox'ı temizleme fonksiyonu
        {
        ...
        private void Kullanici_Ekle_Load(object sender, EventArgs e)
        {
            ...
            toolTip1.SetToolTip(this.txtTcNo, "TC Kimlik Numarası 11 sayıdan oluşmalıdır.");    

            DateTime date = DateTime.Now;    // DateTime'ın DateTime.Now özelliği yardımıyla, date örneğinin içine bugünün tarihinin alınması sağlandı                       
            int yil = Convert.ToInt32(date.ToString("yyyy"));   // yil değişkeni oluşturularak bugünün tarihinden yıl kısmı çekildi
            int ay = Convert.ToInt32(date.ToString("MM"));  // ay değişkeni oluşturularak bugünün tarihinden ay kısmı çekildi.
            int gun = Convert.ToInt32(date.ToString("dd"));  // gun değişkeni oluşturularak bugünün tarihinden gün kısmı çekildi

            dateTimePickerDogumTarihiniz.MinDate = new DateTime(1900, 1, 1);    // DateTimePicker'ın içinde bulunacak en düşük tarih, DateTimePicker'ın MinDate özelliği yardımıyla ayarlandı
            dateTimePickerDogumTarihiniz.MaxDate = new DateTime(yil - 18, ay, gun);    // DateTimePicker'ın içinde bulunacak en yüksek tarih, yukarıda oluşturulan yil, ay ve gun değişkenleri yardımıyla bugünün tarihinden 18 yıl öncesi olacak şekilde ayarlandı. Böylece kişinin 18 yaşından büyük olması sağlandı
            dateTimePickerDogumTarihiniz.Format = DateTimePickerFormat.Short;  // DateTimePicker'ın tasarımsal olarak ekranda nasıl gözükeceği ayarlandı
        ...
        {
            if ((int)e.KeyChar >= 48 && (int)e.KeyChar <= 57 || (int)e.KeyChar == 8)    // Klavyeden basılan tuş ASCII'de 48 ile 57 arasındaysa veya 8 ise, yani basılan tuşlar sayıysa veya Backspace tuşu ise
            {                                                                           
                e.Handled = false;    // Tuşa basılması engellenmez    
            }
            else  // Değilse
            {
                e.Handled = true;  // Tuşa basılması engellenir.          
            }
        }
        ...
        private void button3_Click(object sender, EventArgs e)      // kaydetme butonu 
        {
            ...
            if (kayit_var_mi == false)
            {
                ...
                if (txtTcNo.Text.Length == 11 && txtTcNo.Text != "" && txtAdiniz.Text.Length >= 2 && txtAdiniz.Text != "" && txtSoyadiniz.Text.Length >= 2 && txtSoyadiniz.Text != "" && txtKullaniciAdiniz.Text != "" && txtParolaniz.Text != "" && txtMesleginiz.Text != "" && picBoxFotoEkle.Image != null)
                {
                    ...
                    {
                        baglanti.Open();
                        SqlCommand eklekomut = new SqlCommand("insert into Tbl_Kullanicilar values ('" + txtTcNo.Text + "', '" + txtAdiniz.Text + "', '" + txtSoyadiniz.Text + "', '" + cinsiyet + "' , '" + txtKullaniciAdiniz.Text + "' , '" + txtParolaniz.Text + "' , '" + yetki + "' , '" + txtMesleginiz.Text + "' , '" + dateTimePickerDogumTarihiniz.Value.ToString("yyyy/MM/dd") + "' )", baglanti);   // Textboxlardan, radiobuttondan ve DateTimePickerlardan alınan değerler, 'insert' komutu yardımıyla sırayla Tbl_kullanicilar tablosuna kaydedildi
                        eklekomut.ExecuteNonQuery();  // Yukarıda yazılan sorgunun gerçekleştirilmesi sağlandı
                        baglanti.Close();
                        if (!Directory.Exists(Application.StartupPath + "\\kullaniciresimler"))    // bin klasörü içindeki Debug klasöründe kullaniciresimler klasörü yoksa
                        {
                            Directory.CreateDirectory(Application.StartupPath + "\\kullaniciresimler");   // Dosyayı oluşturur
                                                                                                          
                            picBoxFotoEkle.Image.Save(Application.StartupPath + "\\kullaniciresimler\\" + txtTcNo.Text + ".png");   // picBoxFotoEkle'deki resmi kullaniciresimler dosyasına png formatında olarak kaydeder. İsmi de txtTcNo'ya yazılan TC Kimlik Numarası olur

                            MessageBox.Show("Kayıt başarıyla oluşturuldu", "Metrekare Emlak", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else  // kullaniciresimler klasörü varsa
                        {
                            picBoxFotoEkle.Image.Save(Application.StartupPath + "\\kullaniciresimler\\" + txtTcNo.Text + ".png");   // picBoxFotoEkle'deki resmi kullaniciresimler dosyasına png formatında olarak kaydeder. İsmi de txtTcNo'ya yazılan TC Kimlik Numarası olur
                            MessageBox.Show("Kayıt başarıyla oluşturuldu", "Metrekare Emlak", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
