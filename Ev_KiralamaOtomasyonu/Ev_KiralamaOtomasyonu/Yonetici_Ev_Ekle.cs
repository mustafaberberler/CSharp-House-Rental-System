...
namespace Ev_KiralamaOtomasyonu
{
    public partial class Yonetici_Ev_Ekle : Form
    {
        ...
        private void Ev_Ekle_Load(object sender, EventArgs e)
        {
            ...
            picBoxKullaniciFoto.Width = 150;    // giriş yapan kişinin resminin ayarları
            picBoxKullaniciFoto.Height = 150;

            picBoxEvFoto.Width = 208;   // eklenecek evin fotoğrafının ayarları
            ...
            {
                baglanti.Open();
                SqlCommand combo1sorgu = new SqlCommand("select il from Tbl_iller", baglanti);  // Tbl_iller tablosundan iller seçildi
                SqlDataReader iloku = combo1sorgu.ExecuteReader(); // Bu illerin DataReader'a doldurulması sağlandı
                while (iloku.Read())
                {
                    comboBoxIl.Items.Add(iloku["il"].ToString());   // Bu illerin comboBoxIl combobox'ına yazdırılması sağlandı
                }
                baglanti.Close();
            }
            catch (Exception errormsg)
        ...
        {
            comboBoxIlce.Items.Clear();  // yeni il seçimi yapıldığında eskiler silinmesi için kullanıldı
            try
            {
                baglanti.Open();
                SqlCommand sorgu = new SqlCommand("select ilce from Tbl_ilceler where filtre='" + comboBoxIl.SelectedItem + "'", baglanti);  // Seçilen ilin ilçelerinin Tbl_ilceler'den bulunması sağlandı
                SqlDataReader ilceoku = sorgu.ExecuteReader();    // Bulunan ilçelerin DataReader'a doldurulması sağlandı
                while (ilceoku.Read())
                {
                    comboBoxIlce.Items.Add(ilceoku["ilce"].ToString());  // Bulunan ilçelerin comboBoxIlce combobox'ına doldurulması sağlandı.
                }
                baglanti.Close();
            }
            catch (Exception errormsg)
