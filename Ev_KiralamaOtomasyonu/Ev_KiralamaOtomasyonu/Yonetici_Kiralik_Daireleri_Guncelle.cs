...
namespace Ev_KiralamaOtomasyonu
{
    public partial class Yonetici_Kiralik_Daireleri_Guncelle : Form
    {
        ...
        private void Yonetici_Kiralik_Daireleri_Guncelle_Load(object sender, EventArgs e)
        {
            ...
            Evleri_Listele();
            picBoxKullaniciGiris.Width = 150;
            picBoxKullaniciGiris.Height = 150;
            pictureBox2.Width = 208;
            pictureBox2.Height = 208;
        ...
        private void button3_Click(object sender, EventArgs e)  // kira durumu güncelleme butonu
        {
            ...
            {                
                if (radioButton2.Checked == true)    // kiralanmadı seçilirse
                {
                    kiralik = 0;
                    baglanti.Open();          
                    SqlCommand kiraci_var_mi_sorgu = new SqlCommand("select * from Tbl_kiralananEvler where ilanno='" + txtIlanNo.Text + "'", baglanti);  // Kiracıların gösterildiği tabloda eğer bu evde kiracı varsa onu arar
                    ...
                    baglanti.Close();
                    if (kiraci_var_mi == true)    
                    {
                        try
                        {
                            baglanti.Open();
                            SqlCommand kiraci_sil_sorgu = new SqlCommand("delete from Tbl_kiralananEvler where ilanno='" + txtIlanNo.Text + "'", baglanti);  // kiracı varsa kiracıyı da Tbl_kiralananEvler tablosundan siler
                ...
                baglanti.Open();
                SqlCommand guncellesorgu = new SqlCommand("update Tbl_evler set kiralik='" + kiralik + "' where ilanno='" + txtIlanNo.Text + "'", baglanti);   // Tbl_evler tablosunda, mevcut evin kiralık durumunu 0 yapar, yani SQL tablosunda oluşturulan bit değişkenini False'a döndürür. Yani evin kiralanmamış olmasını sağlar.
